            ...
            <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="w-full flex justify-between flex-wrap">
                    ...
                    <div class="w-full p-4">
                        ...
                        <select name="business" @change="getEmployees" v-model="selectedBusiness" class="form-select w-full bg-transparent rounded focus:border-secondary focus:ring focus:ring-secondary focus:ring-opacity-50 uppercase">
                            <option :value="businees" v-for="(businees, index) in businesses" :key="index">{{ businees.business_name }}</option>
                        </select>
                    ...
                    <div class="w-full p-4">
                        ...
                        <select name="position" @change="getEmployees" v-model="selectedPosition" class="form-select w-full bg-transparent rounded focus:border-secondary focus:ring focus:ring-secondary focus:ring-opacity-50 uppercase">
                            <option :value="position" v-for="(position, index) in positions" :key="index">{{ position.description }}</option>
                        </select>
                    ...
                    <div class="w-full p-4">
                        ...
                        <select v-if="employees.length > 0" name="position" v-model="selectedEmployee" class="form-select w-full bg-transparent rounded focus:border-secondary focus:ring focus:ring-secondary focus:ring-opacity-50 uppercase">
                            <option :value="employee" v-for="(employee, index) in employees" :key="index">{{ employee.name }}</option>
                        </select>
                    ...
                    <div class="w-full p-4">
                        ...
                        <small>*El primer documento será el principal y los siguientes serán guardados como anexos</small>
...
export default {
  ...
  setup(props) {
    ...
    {
        const raw = await fetch(route('business.position.users',{ business : selectedBusiness.value.id, position : selectedPosition.value.id }))
        const response = await raw.json()
        employees.value = response 
        selectedEmployee.value = employees.value[0] 
        form.reviewers = form.reviewers.filter(rev => {
            return (rev.business_id === selectedBusiness.value.id) || rev.business_id === 1 || selectedBusiness.value.id === 1
        } )      
    }
